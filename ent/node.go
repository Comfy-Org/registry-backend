...
func (*Node) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case node.FieldID, node.FieldRawID, node.FieldPublisherID, node.FieldName, node.FieldDescription, node.FieldCategory, node.FieldAuthor, node.FieldLicense, node.FieldRepositoryURL, node.FieldIconURL, node.FieldStatus, node.FieldStatusDetail:
