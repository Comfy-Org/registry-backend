...
func (uq *UserQuery) QueryPublisherPermissions() *PublisherPermissionQuery {
	query := (&PublisherPermissionClient{config: uq.config}).Query()
...
func (uq *UserQuery) QueryReviews() *NodeReviewQuery {
	query := (&NodeReviewClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withPublisherPermissions: uq.withPublisherPermissions.Clone(),
...
func (uq *UserQuery) WithPublisherPermissions(opts ...func(*PublisherPermissionQuery)) *UserQuery {
	query := (&PublisherPermissionClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withPublisherPermissions = query
	return uq
}
...
func (uq *UserQuery) WithReviews(opts ...func(*NodeReviewQuery)) *UserQuery {
	query := (&NodeReviewClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		_spec       = uq.querySpec()
		loadedTypes = [2]bool{
			uq.withPublisherPermissions != nil,
			uq.withReviews != nil,
		}
	)
	...
	if query := uq.withPublisherPermissions; query != nil {
		if err := uq.loadPublisherPermissions(ctx, query, nodes,
			func(n *User) { n.Edges.PublisherPermissions = []*PublisherPermission{} },
			func(n *User, e *PublisherPermission) {
	...
	if query := uq.withReviews; query != nil {
		if err := uq.loadReviews(ctx, query, nodes,
			func(n *User) { n.Edges.Reviews = []*NodeReview{} },
			func(n *User, e *NodeReview) { n.Edges.Reviews = append(n.Edges.Reviews, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadPublisherPermissions(ctx context.Context, query *PublisherPermissionQuery, nodes []*User, init func(*User), assign func(*User, *PublisherPermission)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(publisherpermission.FieldUserID)
	}
	query.Where(predicate.PublisherPermission(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.PublisherPermissionsColumn), fks...))
	}))
...
func (uq *UserQuery) loadReviews(ctx context.Context, query *NodeReviewQuery, nodes []*User, init func(*User), assign func(*User, *NodeReview)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(nodereview.FieldUserID)
	}
